...
######操作系统iOS中应用程序使用的内存不是统一分配空间，代码运行使用的空间在3个不同的内存区域。
...
######操作系统使用stack 段中的指针值访问heap 段中的对象。如果stack 对象的指针没有了，则heap 中的对象就不能访问。这也是内存泄露的原因。</br>
text segment：
