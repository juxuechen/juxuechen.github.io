...
/**
 初始化该蹦床需要使用一个协议和一组原始观察者。然后，你可以添加或者删除观察者。如果观察者实现了协议，协议中定义的任何方法都将被转发至当前所有观察者。
 **/
...
/**
 你可以覆盖methodSignatureForSelector:方法。Objective-C的消息分发器使用该方法为未知的选择器构建一个NSInvocation。
